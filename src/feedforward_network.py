...
print(f'Número de valores de C: {len(C_values)}, Número de valores de L: {len(L_values)}')
...
print(f'Número de datos en el dataset: {N}')

# Partición de datos (75% entrenamiento, 25% test)
train_size = int(0.75 * N)
test_size = N - train_size
train_dataset, test_dataset = random_split(mnist, [train_size, test_size])
print(f'Número de datos de entrenamiento: {len(train_dataset)}, Número de datos de prueba: {len(test_dataset)}')

# Cargar datos en DataLoader
train_loader = DataLoader(train_dataset, batch_size=len(train_dataset), shuffle=True)
test_loader = DataLoader(test_dataset, batch_size=len(test_dataset), shuffle=True)
print('Datos cargados en DataLoaders.')
...
print('Datos redimensionados a 2D.')

# Normalización min-max
X_train = (X_train - X_train.min()) / (X_train.max() - X_train.min())
X_test = (X_test - X_test.min()) / (X_test.max() - X_test.min())
print('Datos normalizados usando min-max scaling.')

# One-hot encoding de etiquetas
Y_train = F.one_hot(y_train, J).float()
Y_test = F.one_hot(y_test, J).float()
print('Etiquetas convertidas a one-hot encoding.')
...
print(f'Tamaño de conjunto de entrenamiento para validación: {len(X_train_val)}, Tamaño de validación: {len(X_val)}')

# Matriz de rendimiento
Performance = torch.zeros(len(C_values), len(L_values))
print(f'Número total de iteraciones para la búsqueda de hiperparámetros: {len(C_values) * len(L_values)}\n')
...
for i, C in enumerate(C_values):
    for j, L in enumerate(L_values):
        print(f'Iteración {i * len(L_values) + j + 1}: Evaluando C={C}, L={L}')
        
        # Generar pesos aleatorios para la capa oculta
        W_hidden = torch.randn(X_train_val.size(1), L)
        print(f'Pesos de la capa oculta generados: {W_hidden.shape}')
        ...
        print('Pesos de salida calculados.')
...
Copt = C_values[i]
Lopt = L_values[j]
print(f'Mejores hiperparámetros encontrados: C = {Copt}, L = {Lopt}')
...
print('Entrenamiento final completado con los mejores hiperparámetros.')
...
print(f'CCR en el conjunto de prueba: {CCR_test}')
